...
public class PSerial implements SerialPortEventListener, iSerial {
	...
	static{
		MCBMiniNativeLoader.loadNativeLibrary();
	}
	...
	public PSerial(String iname, int irate,
		...
		if( iname == null || iname.length() == 0 ){
			throw new RuntimeException("Port name needs to be valid");
		}
		...
		if (serialEvent.getEventType() == SerialPortEvent.DATA_AVAILABLE) {
			...
			synchronized (event_handlers) {
				for (SerialEventHandler handler : event_handlers) {
					handler.handleSerialDataAvailableEvent(this);
				}
			}
		}
	...
	/**
	 * Finds a serial port that contains a substring in its name
	 * @param part_of_name
	 * @return the name of the serial port
	 */
	public static String findPort(String part_of_name){
		String ret = null;
		...
		if( ret == null ){
			//			throw new RuntimeException("Couldn't find a serial port");
		}
